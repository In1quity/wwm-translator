...
def read_dat_document(dat_path: Path) -> DatDocument | None:
    with dat_path.open("rb") as fh:
        ...
        work_blocks = struct.unpack("<I", header_data[4:8])[0] if len(header_data) >= 8 else count_full
...
def write_dat_document(output_path: Path, document: DatDocument) -> Path:
    ...
    header_data = bytes.fromhex(document.header_hex) if document.header_hex else _default_header(
        document.count_full, document.work_blocks
    )
