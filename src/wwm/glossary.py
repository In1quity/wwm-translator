    ...
    if glossary_path.is_file():
        with glossary_path.open("r", encoding="utf-8-sig", newline="") as fh:
            ...
            for row in reader:
                ...
                payload.append(
                    (cn.strip(), en.strip(), target.strip(), category.strip(), int(strict or 0))
                )
                count += 1
            if payload:
                conn.executemany(
                    "INSERT INTO glossary(cn, en, target, category, strict) VALUES(?, ?, ?, ?, ?)",
                    payload,
                )
    conn.commit()
    conn.close()
...
def export_glossary_from_db(db_path: Path, glossary_path: Path) -> dict[str, int]:
    conn = open_db(db_path)
    rows = conn.execute(
        "SELECT cn, en, target, category, strict FROM glossary ORDER BY category, en, cn"
    ).fetchall()
    ...
    with glossary_path.open("w", encoding="utf-8-sig", newline="") as fh:
        ...
        for row in rows:
            writer.writerow(
                [row["cn"], row["en"], row["target"], row["category"], str(row["strict"])]
            )
    conn.close()
